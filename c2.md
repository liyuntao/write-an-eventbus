...
1. 按照上述定义，register方法用于将订阅者对象的方法注册至EventBus当中。所以EventBus对象当中需要保存这个订阅者对象，同时查找出此对象中所有被注解标注的订阅方法。这里订阅者对象和其上的所有方法具有一对多的K-V的关系，需要用到`Map<Object, List<Method>>`这样一个集合类。
